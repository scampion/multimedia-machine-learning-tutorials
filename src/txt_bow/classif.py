...
lab_train, lab_test, vec_test = [pickle.load(open(file)) 
                                 for file 
...
clf = pickle.load(open('classifier.pik'))

results = [ int(clf.predict(v)) for v in vec_test ]
np.testing.assert_array_almost_equal(results,lab_test)
